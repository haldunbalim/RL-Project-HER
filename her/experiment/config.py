...
from her.naf_utils.exploration import *
from her.naf import NAF
...
def configure_naf(dims, params, reuse=False, use_mpi=True, clip_return=True):
    sample_her_transitions = configure_her(params)
    # Extract relevant parameters.
    gamma = params['gamma']
    params["scope"]="naf"
    ...
    env = cached_make_env(params['make_env'])
    env.reset()
    naf_params.update({'input_dims': input_dims,  # agent takes an input observations
                        'T': params['T'],
                        'clip_pos_returns': True,  # clip positive returns
                        'clip_return': (1. / (1. - gamma)) if clip_return else np.inf,  # max abs of return
                        'rollout_batch_size': rollout_batch_size,
    ...
    policy = NAF(reuse=reuse, **naf_params, use_mpi=use_mpi)
    return policy
